...
class EEGDataset():
    ...
    def load_data(self):
        ...
        for dir in tqdm(dirnames, desc="loading captions"):
            # print("dir:",dir)
        ...
        if self.classes is not None and self.pictures is not None:
            ...
            for i in range(len(self.classes)):
                ...
                if class_idx < len(all_folders):
                    ...
                    print("images folder path:",folder_path)
            ...
            for folder in all_folders:
                folder_path = os.path.join(img_directory, folder)
                all_images = [img for img in os.listdir(folder_path) if img.lower().endswith(('.png', '.jpg', '.jpeg'))]
                # for image in tqdm(all_images, desc='Getting all images and their captions'):
                #     # we gen captions for each here
                all_images.sort()  
                images.extend(os.path.join(folder_path, img) for img in all_images)
        else:
        ...
        for subject in tqdm(self.subjects, desc=f"Loading EEG data (Train={self.train})", unit="subject"):
            if self.train:
    ...
    def Textencoder(self, text):   
            ...
            text_inputs = torch.cat([clip.tokenize(t) for t in tqdm(text, desc="Tokenizing text")]).to(device)
    ...
    def ImageEncoder(self,images):
        ...
        for i in tqdm(range(0, len(images), batch_size), desc="Encoding Images", unit="batch"):
            batch_images = images[i:i + batch_size]
            image_inputs = torch.stack([preprocess_train(Image.open(img).convert("RGB")) for img in batch_images]).to(device)

            with torch.no_grad():
                # print("test for image_inputs:", image_inputs.shape)
                batch_image_features = vlmodel.encode_image(image_inputs)
                # print("Test for the batch_image_feature value:", batch_image_features.shape)
    ...
    def __getitem__(self, index):
        # 1. Get the EEG data and its ground truth class label.
        # This works perfectly for multi-subject data as these are flat, concatenated tensors.
        x = self.data[index]
        label = self.labels[index]  # This is a tensor, e.g., tensor(123)
    
        # 2. Derive the text_index directly from the label.
        # The class index IS the text index. We use .item() to get the Python integer.
        text_index = label.item()
    
        # 3. Derive the img_index based on the class and the sample's position.
        # To make this clear, let's use the parameters from your data structure.
        samples_per_class_train = 10
        chunks_per_sample_train = 4
    
        if self.train:
            # We need to figure out which of the 10 images for this class this EEG sample belongs to.
            # The image trial changes every `chunks_per_sample_train` (4) steps.
            # The pattern of 10 images repeats for every class.
            
            # This tells us which image trial we are on globally (0, 1, 2, ...).
            global_image_trial_index = index // chunks_per_sample_train
            
            # This tells us which image trial we are on *within this class* (0 to 9).
            image_offset_in_class = global_image_trial_index % samples_per_class_train
            
            # The final image index is the start of the class's images plus this offset.
            img_index = text_index * samples_per_class_train + image_offset_in_class
        
        else: # Test mode
            # In your test set, there is only one image per class.
            # Therefore, the image index is the same as the class (text) index.
            img_index = text_index
    
        # 4. Retrieve all associated data using the correctly derived indices.
        # This will now work for any number of subjects.
        text = self.text[text_index]
        img = self.img[img_index]
