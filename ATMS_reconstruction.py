...
def evaluate_model(sub, eeg_model, dataloader, device, text_features_all, img_features_all, k, config):
    ...
    with torch.no_grad():
        for batch_idx, (eeg_data, labels, text, text_features, img, img_features) in enumerate(dataloader):
            ...
            all_eeg_features.append(eeg_features.cpu())
    ...
    final_eeg_features = torch.cat(all_eeg_features, dim=0)
    final_true_labels = torch.cat(all_true_labels, dim=0)

    return average_loss, accuracy, top5_acc, final_eeg_features, final_true_labels
...
#     alpha = 0.99
#     all_labels_set = set(range(text_features_all.size(0)))
#     mse_loss_fn = nn.MSELoss()
...
#             # Since test batch size is 1, this loop runs once per sample
#             for idx, true_label_tensor in enumerate(labels):
#                 true_label = true_label_tensor.item()
                
#                 # --- This is the K-way classification setup ---
#                 possible_classes = list(all_labels_set - {true_label})
#                 # Ensure we don't try to sample more than available
#                 num_to_sample = min(k - 1, len(possible_classes))
                
#                 selected_decoys = random.sample(possible_classes, num_to_sample)
#                 selected_indices = selected_decoys + [true_label]
#                 random.shuffle(selected_indices) # Shuffle to avoid position bias
                
#                 # The index of the true label within our shuffled k-way setup
#                 correct_k_way_index = selected_indices.index(true_label)

#                 selected_img_features = img_features_all[selected_indices]
                
#                 # --- Logit Calculation ---
#                 logits_single = logit_scale * eeg_features[idx] @ selected_img_features.T

#                 # --- Top-K Accuracy Calculation ---
#                 # Determine how many 'k' to check for topk (can't be more than the number of classes)
#                 top_k_val = min(k, 5)
                
#                 _, top_indices = torch.topk(logits_single, k=top_k_val, largest=True)
                
#                 # Check for Top-1 hit
#                 if top_indices[0] == correct_k_way_index:
#                     top1_correct += 1
                
#                 # Check for Top-2 hit
#                 if correct_k_way_index in top_indices[:2]:
#                     top2_correct += 1
                
#                 # Check for Top-5 hit
#                 if correct_k_way_index in top_indices: # top_indices already contains top 5 (or fewer)
#                     top5_correct += 1
...
#     return average_loss, top1_acc, top2_acc, top5_acc
...
def main_train_loop(sub, current_time, eeg_model, train_dataloader, test_dataloader, optimizer, device, text_features_train_all, text_features_test_all, img_features_train_all, img_features_test_all, config, logger=None):
    ...
    final_epoch_test_features = None
    final_epoch_test_labels = None
    ...
    for epoch in range(config.epochs):
        ...
        # Evaluate the model
        test_loss, test_accuracy, top5_acc, _, _ = evaluate_model(sub, eeg_model, test_dataloader, device, text_features_test_all, img_features_test_all,k=200, config=config)
        _, v2_acc, _, _, _ = evaluate_model(sub, eeg_model, test_dataloader, device, text_features_test_all, img_features_test_all, k = 2, config=config)
        _, v4_acc, _, _, _ = evaluate_model(sub, eeg_model, test_dataloader, device, text_features_test_all, img_features_test_all, k = 4, config=config)
        _, v10_acc, _, _, _ = evaluate_model(sub, eeg_model, test_dataloader, device, text_features_test_all, img_features_test_all, k = 10, config=config)
        _, v50_acc, v50_top5_acc, _, _ = evaluate_model(sub, eeg_model, test_dataloader, device, text_features_test_all, img_features_test_all,  k=50, config=config)
        _, v100_acc, v100_top5_acc, eeg_inference_features, eeg_inference_labels = evaluate_model(sub, eeg_model, test_dataloader, device, text_features_test_all, img_features_test_all,  k=100, config=config)
        ...
        if test_accuracy > best_accuracy:
            ...
            best_epoch_features = eeg_inference_features
    ...
    # logger.finish()
    return results, best_epoch_features, best_epoch_labels
...
# def main_train_loop(sub, current_time, eeg_model, train_dataloader, test_dataloader, optimizer, device, text_features_train_all, text_features_test_all, img_features_train_all, img_features_test_all, config, logger=None):
    ...
    #     if (epoch + 1) % 5 == 0:
    #         save_dir = f"/ibex/user/thagafhh/data/models/contrast/{config.encoder_type}/{sub}/{current_time}" if config.insubject else f"/ibex/user/thagafhh/data/models/contrast/across/{config.encoder_type}/{current_time}"
    #         os.makedirs(save_dir, exist_ok=True)
    ...
    #     # Evaluate the model with k=200 (or your desired max k)
    #     test_loss, top1_acc, top2_acc, top5_acc = evaluate_model(sub, eeg_model, test_dataloader, device, text_features_test_all, img_features_test_all, k=200, config=config)
        
    #     test_losses.append(test_loss)
    #     test_top1_accuracies.append(top1_acc)
    #     test_top2_accuracies.append(top2_acc)
    #     test_top5_accuracies.append(top5_acc)
        
    #     # Append results for this epoch to the list for CSV saving
    #     epoch_results = {
    #         "epoch": epoch + 1,
    #         "train_loss": train_loss,
    #         "train_accuracy": train_accuracy,
    ...
    #         best_epoch_info = epoch_results.copy()

    #     # Updated print statement
    #     print(f"Epoch {epoch + 1}/{config.epochs} - Train Loss: {train_loss:.4f}, Train Acc: {train_accuracy:.4f}")
    #     print(f"  Test Loss: {test_loss:.4f} | Top-1: {top1_acc:.4f} | Top-2: {top2_acc:.4f} | Top-5: {top5_acc:.4f}")
  
    # # --- Plotting section (simplified for clarity, your original plotting code is fine) ---
    ...
    # axs[0, 1].plot(train_accuracies, label='Train Accuracy')
    # axs[0, 1].plot(test_top1_accuracies, label='Test Top-1 Accuracy')
    # axs[0, 1].legend()
    # axs[0, 1].set_title("Top-1 Accuracy Curve")

    # axs[1, 0].plot(test_top2_accuracies, label='Test Top-2 Accuracy', color='green')
    # axs[1, 0].legend()
    # axs[1, 0].set_title("Top-2 Accuracy Curve")

    # axs[1, 1].plot(test_top5_accuracies, label='Test Top-5 Accuracy', color='purple')
    # axs[1, 1].legend()
    # axs[1, 1].set_title("Top-5 Accuracy Curve")
...
def main():
    ...
    for sub in subjects:
        ...
        results, inference_features, inference_labels = main_train_loop(sub, current_time, eeg_model, train_loader, test_loader, optimizer, device, 
        ...
        if inference_features is not None and inference_labels is not None:
                    results_dir = os.path.join(args.output_dir, args.encoder_type, sub, current_time)
                    features_save_path = os.path.join(results_dir, f"{sub}_best_epoch_features.pt")
                    
                    torch.save({
                        'eeg_features': inference_features,
                        'labels': inference_labels
                    }, features_save_path)
                    
                    print(f"Best epoch's test EEG features saved to {features_save_path}")
